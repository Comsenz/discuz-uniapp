<template>
  <view class="audio">
    <view class="audio-cover" :class="{ hasbg: !poster }">
      <image :src="poster" v-if="poster" class="audio-cover__img"></image>
      <view class="audio-cover__btn" @click="handleBtnClick"></view>
    </view>
    <view class="audio-con">
      <text class="audio-con__info-title am-text-eill">{{ name }}</text>
      <text class="audio-con__time">{{ audioTimeUpdate }}</text>
    </view>
  </view>
</template>
  ...
  methods: {
    ...
    contextInit() {
      ...
      innerAudioContext.onPlay(() => {
        that.audioTimeUpdate = this.sToHs(Math.floor(innerAudioContext.currentTime));
      });
