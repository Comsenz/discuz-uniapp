// 基于此 https://github.com/reststate/reststate-vuex，增加了一个 included 数据处理
...
const storeIncluded = ({ commit, dispatch }, result, resourceName) => {
  if (result.included) {
    // store the included records
    result.included.forEach(relatedRecord => {
      // 赋值 parent 字段好用户数据的处理
      relatedRecord.parentType = resourceName;
      // 修复 dispatch action 报错问题，源代码这里是取的 relatedRecord.type
      const action = `${resourceName}/storeRecord`;
    ...
    allRecords.forEach(primaryRecord => {
      if (primaryRecord.relationships) {
        Object.keys(primaryRecord.relationships).forEach(relationshipName => {
          ...
          const action = `${resourceName}/storeRelated`;
...
const matches = criteria => test =>
  Object.keys(criteria).every(key => {
    return deepEquals(criteria[key], test[key])
  });

// 增加一个针对于 included 数据获取对应模块下面的对应关系的所有数据
const matchesIncluded = criteria => test => {
  if (criteria && criteria.parent === test.parentType
    && criteria.relationship === test.type) return true;
  return false;
};
...
const resourceModule = ({ name: resourceName, httpClient }) => {
  ...
  return {
      ...
      loadAll({ commit, dispatch }, { options } = {}) {
        ...
        return client
          ...
          .then(result => {
            ...
            storeIncluded({ commit, dispatch }, result, resourceName);
          })
      ...
      loadById({ commit, dispatch }, { id, options }) {
        ...
        return client
          ...
          .then(results => {
            ...
            storeIncluded({ commit, dispatch }, results, resourceName);
          })
          .catch(handleError(commit));
      },
      ...
      loadPage({ commit, dispatch }, { options }) {
        ...
        return client
          ...
          .then(response => {
            ...
            storeIncluded({ commit, dispatch }, response, resourceName);
          })
          .catch(handleError(commit));
      },
    ...
    getters: {
      ...
      included: state => params => {
        const related = state.records.filter(matchesIncluded(params));
        if (!related) {
          return null;
        }
        return related;
